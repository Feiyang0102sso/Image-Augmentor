...
def load_augmentor_from_file_object(config_file_object):
    """
    Loads augmentor from a file-like object (like an uploaded file or a file opened from disk).
...
uploaded_files = st.sidebar.file_uploader("Upload Image(s)", type=['png', 'jpg', 'jpeg'],
...
if st.sidebar.button("🚀 Run Analysis", type="primary", use_container_width=True, disabled=(not model or not classes)):
    ...
    # 1. The only requirement to run is having images.
    if not uploaded_files:
        st.warning("Please upload at least one image.")
        st.stop()  # Stop execution if no images are provided.

    # 2. Decide which config to use: the uploaded one or the default one.
    config_to_use = None
    if uploaded_config is not None:
        # If user uploaded a file, use it.
        config_to_use = uploaded_config
        logging.info(f"Using uploaded configuration file: {uploaded_config.name}")
    else:
        # If no file was uploaded, try to use the default 'config.json'.
        try:
            # We need to open it and wrap it in a BytesIO object so it behaves like an uploaded file.
            with open("config.json", "rb") as f:
                config_to_use = BytesIO(f.read())
            logging.info("Using default configuration file: config.json")
        except FileNotFoundError:
            st.error("Operation failed: No config file uploaded and default 'config.json' was not found.")
            st.stop()  # Stop execution if no config can be found.

    # 3. Proceed with the analysis using the selected config.
    session_logs = []
    setup_logging(session_logs)
    files_to_process = uploaded_files if isinstance(uploaded_files, list) else [uploaded_files]

    with st.spinner("Processing... please wait."):
        # We pass `config_to_use` which is either the uploaded file or the BytesIO wrapper.
        ...
        if augmentor:
            ...
            for file in files_to_process:
                # The name attribute is missing from BytesIO, so we handle it gracefully for logging
                file_name = getattr(file, 'name', 'default config')
